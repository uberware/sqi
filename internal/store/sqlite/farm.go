...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
RETURNING id, name, description, max_concurrent_tasks,
	max_attempts, retry_delay_seconds, failure_limit, created_at, updated_at`

	sqlGetFarm = `
SELECT id, name, description, max_concurrent_tasks,
	max_attempts, retry_delay_seconds, failure_limit, created_at, updated_at
FROM farms WHERE id = ?`
...
FROM farms ORDER BY name`
	...
	sqlUpdateFarm = `
UPDATE farms
SET name = ?, description = ?, max_concurrent_tasks = ?,
...
func scanFarm(row scanner) (store.Farm, error) {
	...
	if err := row.Scan(&f.ID, &f.Name, &f.Description, &f.MaxConcurrentTasks,
		&maxAtt, &delay, &failLim, &createdAt, &updatedAt); err != nil {
		return store.Farm{}, err
	}
	f.MaxAttempts, f.RetryDelaySeconds, f.FailureLimit = intPtr(maxAtt), intPtr(delay), intPtr(failLim)
...
func (s *Store) CreateFarm(ctx context.Context, farm store.Farm) (store.Farm, error) {
	...
	row := s.stmtInsertFarm.QueryRowContext(ctx,
		farm.ID, farm.Name, farm.Description, farm.MaxConcurrentTasks,
		nullInt(farm.MaxAttempts), nullInt(farm.RetryDelaySeconds), nullInt(farm.FailureLimit), now, now)
...
func (s *Store) UpdateFarm(ctx context.Context, farm store.Farm) (store.Farm, error) {
	...
	row := s.stmtUpdateFarm.QueryRowContext(ctx,
		farm.Name, farm.Description, farm.MaxConcurrentTasks,
		nullInt(farm.MaxAttempts), nullInt(farm.RetryDelaySeconds), nullInt(farm.FailureLimit), now, farm.ID)
	out, err := scanFarm(row)
	return out, mapErr(err)
}
