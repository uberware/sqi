...
//
// The mode is passed to the open itself rather than chmod'd afterwards, so the
// file never exists at a wider mode with secret bytes already in it. umask can
// only ever REMOVE bits from an open mode, never add them, so a permissive
// umask cannot widen this; a restrictive one that strips owner bits would make
// the caller's own subsequent write fail loudly rather than silently exposing
// anything.
func createSecret(path string) (*os.File, error) {
	return os.OpenFile(path, os.O_WRONLY|os.O_CREATE|os.O_TRUNC|syscall.O_NOFOLLOW, SecretFileMode.Perm())
}
