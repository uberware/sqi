...
export function parsePathDeliveries(template: string, format: 'yaml' | 'json'): PathTranslation | null {
...
export function serializePathDeliveries(
  ...
  const doc = (format === 'json' ? JSON.parse(template) : parseYaml(template)) as Record<string, unknown>;
