...
func TestInstanceName_FitsTheDNSLabelLimit(t *testing.T) {
	...
	// Two tests must never advertise the same name: these run on real networks.
	if other := instanceName(t, "prefix"); other == instanceName(t, "different") {
		t.Error("different prefixes produced the same instance name")
	}
}
