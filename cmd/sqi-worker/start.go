...
func runStart(cmd *cobra.Command, _ []string) error {
	...
	applyDiscoveredTLS(ctx, &cfg, found, logger)
...
// applyDiscoveredTLS folds what mDNS learned about the server into cfg.
//
...
func applyDiscoveredTLS(ctx context.Context, cfg *workerconfig.WorkerConfig, found workerdiscovery.Result, logger *slog.Logger) {
	switch {
	case found.NATSTLS && cfg.NATS.TLSEnabled == workerconfig.TLSOff:
	...
	case found.NATSTLS && !cfg.NATS.UseTLS(false):
		...
		logger.InfoContext(ctx,
			...
			slog.String("url", cfg.NATS.URL))
		cfg.NATS.TLSEnabled = workerconfig.TLSOn
		if cfg.NATS.TLSCAFile == "" && !cfg.NATS.InsecureSkipVerify {
			logger.WarnContext(ctx,
	...
	if found.HTTPTLS && strings.HasPrefix(cfg.NATS.ServerURL, "http://") {
		logger.WarnContext(ctx,
			...
			slog.String("server_url", cfg.NATS.ServerURL))
	}
}
