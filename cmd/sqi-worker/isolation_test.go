...
func TestEffectiveSessionRoot_RootDefaultsUnderVarLib(t *testing.T) {
	cfg := workerconfig.WorkerSettings{DataDir: "/data"}

	got, mode := effectiveSessionRootFor(cfg, func() bool { return true })

	if got != defaultRootSessionDir {
		t.Errorf("got %q, want %q (data_dir must never be reused as the session root)", got, defaultRootSessionDir)
	}
	if mode != 0o711 {
		t.Errorf("mode = %o, want 0711 (traversable from birth — the only worker kind that can actually isolate)", mode)
	}
}
...
// self-inflicted boot refusal a prior round introduced: defaultRootSessionDir
// (/var/lib/sqi-worker-sessions) must never be a descendant of
// workerconfig.Default().Worker.DataDir, in EITHER the HOME-set or
// HOME-unset case. workerconfig.LoadOrCreateWorkerID creates DataDir at 0700
// by design; if the session root nested under it, this package's own
// boot-time isolation.ValidateTraversable would walk up from the session
// root, hit DataDir at 0700, and refuse to start over a directory sqi itself
// had just created — naming the exact self-inflicted bug an earlier revision
// of defaultRootSessionDir (/var/lib/sqi-worker/sessions, nesting under the
// HOME-unset DataDir fallback /var/lib/sqi-worker) introduced.
func TestDefaultSessionRootNeverNestsUnderDefaultDataDir(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if isDescendantPath(dataDir, defaultRootSessionDir) {
				t.Errorf("defaultRootSessionDir %q is a descendant of the default data_dir %q — "+
